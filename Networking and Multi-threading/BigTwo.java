...
the user interface.
	...
	/**
	 * a method from BigTwo to make a connection
	...
	/**
	 * a method for setting the nameArray
	 * 
	 */
	...
	/**
	 * a method for retrieving the nameArray
	...
	public synchronized void makeMove(int playerIdx, int[] cardIdx) {
		...
		if (this.end) {
			this.ui.disable();
			this.ui.repaint();
			c.sendMessage(new CardGameMessage(CardGameMessage.MOVE, -1, cardIdx));
		}
		else if (!this.end) {
			this.ui.setActivePlayer((currentPlayerIdx + 1) % 4);
			this.ui.repaint();
			c.sendMessage(new CardGameMessage(CardGameMessage.MOVE, -1, cardIdx));
		}
